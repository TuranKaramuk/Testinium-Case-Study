...
def setup_logger(output_file=None, logging_config=None):
    ...
    if logging_config is not None:
        ...
        with open(os.path.join(os.path.dirname(__file__), 'logger_config.yaml'),
                  'r') as fh:
...
def write_file(text):
    with open(os.path.join(os.path.dirname(os.getcwd()), 'item_info.txt'), 'a', encoding='utf-8') as f:
        f.write(text + '\n')
        f.close()
